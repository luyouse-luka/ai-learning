...
| **AI 线**（可并行） | 阶段 1 · 第 12-13 天 | 14 天行动清单走到「FastAPI 接口 + 简单 HTML 前端」 |
...
- [~] Day 4 原生 HTML 前端（对照组）—— `week2/api/static/index.html`，5 个 TODO 待填
...
- [~] **前端 AI 集成** —— 流式文本渲染（打字机效果）／ 加载状态处理 ／ 错误重试 ／ 对话历史展示
  - > Day 4 正在做基础版（三态 + 失败分支），流式渲染尚未做
...
- [~] Day 12-13 改造成 FastAPI 接口 + 简单 HTML 前端 —— 后端 ✅ `week2/api/main.py`；前端进行中
