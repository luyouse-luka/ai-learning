...
| 轨道 | 位置 | 说明 |
|---|---|---|
| **主线**（总纲 9 个月） | 阶段一 · **第 1 个月第 1 周**（Week 3，08-09 起） | 工具链就位：nvm + Node **v24.19.0** + pnpm 11.20 + `week3/frontend/`（Vue 3.5 / Vite 8.2 / TS 6.0）。本周做到「Vite 代理 → TS 契约类型 → 第一个组件 → 接通真接口」 |
| **AI 线**（可并行） | 阶段 1 ☑ **收尾完成** → 阶段 2 · **开头** | 14 天清单 Day 1-13 ☑（Day 14 部署暂缓，属阶段二红线）。护栏 08-07 已补齐。Week 3 只做阶段 2 的**头两天**：JSON 解析 + Function Calling 跑通一次（指南给 4 周，不许压） |

**Week 3 排期已定**（08-09 ly 拍板三条）→ 详见 **`week3/README.md`**：
**每天 2-3 小时** ／ **主轨 5:2**（Day 1-2 AI 线，Day 3-7 主线前端重写）／ **目录重构推到 Week 3 Day 7**。
...
- [x] Day 7 巩固日（08-09，**4 件事完成 3 件**，第 4 件明确推到 Week 3 Day 7）
      **① 盲写本周核心 ☑** —— `week2/day7_review.py` 从零写到跑通：读文件 → 切块 → 逐块调 API → 汇总，
      含成本统计 / 日调用护栏 / 截断检测 / Stuff↔Map-Reduce 分叉。
      最大观察：同一份 1297 字符文本**只差一个 `if`**，成本 −39% 而总结**反而更详细**
      （Reduce 那步只见摘要不见原文 → **Map-Reduce 的本质缺陷 = 没有任何一次调用读过全文**）。
      收尾把写死的文件名改成 `sys.argv[1]`（教练代改），4 条路径实跑，**换文件零改码**
      **② Week 2 总复盘 ☑** —— `week2/day7_review.md`，5 条自检逐条作答 + 逐条批改。
      结果 ②③ 过、①⑤ **☑\***（ly 要求教练代填/代改，验收挪 Week 3 Day 1 间隔回顾）、
      ④ **☐ 判「没练过」**（`day2_summarize.py` 只有 `json.dump` 无 `json.loads`，本周确实没排过）。
      **Week 3 解锁**（ly 拍板）：门槛用来拦「夹生」，不是拦「没教过」
      **② 补充（08-09 晚）**：「明天要复习的 1 个概念」由**教练代填**（ly 指派）＝
      **护栏自查两问：谁调用它 / 它判断的状态谁维护、活多久**；
      **3 道复盘题已出，1.5/3**（Q1 半对：漏了 Reduce，**Map-Reduce 调用次数 = 块数+1**；
      Q2 对；Q3 错：`_calls_today` 住在进程内存，每次运行重新 `= 0`，护栏跨不了进程）
      **③ Week 3 计划 ☑** —— `week3/README.md`（约束表 / 7 天表 / 结转的债 / 结束自检）。
      新增一条硬要求针对**新瓶颈 4**：交付前自己先跑路径、写下验过哪些判据，
      判据 = Week 3 结束时「自己独立发现的 bug」不能再是 0
      **④ 目录重构 ☐** —— **08-09 决定推到 Week 3 Day 7**（理由：改路径要连带改
      `week1/CLAUDE.md` / 两份 README / 本文件 / `~/.claude` memory 全部引用，
      一开工就搬家会在最需要专注时制造一堆坏链接）
...
2. **静默 bug 意识**（本周重点）—— Day 5 有实测进展（空 summary 的 502 亲手堵上、亲眼见到它触发），
   ...
   **统一表述：「没有 A」不等于「有 B」——负向判据证明不了正向结论，判据必须直接测你要证的那件事。**
   ...
   → **Day 7 第三次**：`check_daily_limit()` 写得完全正确，但**一次都没工作过**——
   没人调用它，`_calls_today` 也没人 `+= 1`（**两重失效叠一起**，不止「没调用」一重）。
   ✅ **已修复（2026-08-12 复核代码确认）**：`week2/day7_review.py:60` 有调用、`:79` 有 `+= 1`。
   本条记的是 Day 7 **当时**的状态，保留作学习痕迹，**不代表现状**——复习时别照着它找 bug。
   由此定下护栏自查的两个问题：**① 谁调用它？② 它判断的那个状态，谁在维护？**
   一个问入口，一个问数据，**护栏失效只有这两个死法**。
   ⚠️ 本周主线由此定稿为四张脸：**没报错 ≠ 有结果 / 判据没变红 ≠ 功能对 /
   函数写了 ≠ 被调用 / 参数传了 ≠ 生效**
3. **变量作用域**（已第**五**次）—— 同一条规则的五张脸：循环 → `if` 分支 → 函数边界（`+1` 写进 `check_` 里）
   → **`global` 声明的覆盖范围**（删了名字但 `if` 分支里还有 `= 0` → 全新进程首次调用 500 `UnboundLocalError`）
   → **进程边界**（08-09 Day 7 出题 Q3 答错：`_calls_today` 住在进程内存，
   `python xxx.py` 每跑一次都是新进程重新 `= 0`，日调用护栏跨不了进程 → 要拦就得落到文件/SQLite/Redis）。
   **判据：函数里对某个名字有任何赋值（`=` / `+=` / `for x in`）就必须 `global` 声明，不管那行会不会执行到**
   **升级版判据：问一个值「谁在维护」还不够，必须再问一句「它活多久」——函数返回就没了？进程退出就没了？**

4. **能写 ≠ 能自查**（08-09 Day 7 新增）—— 盲写脚本这件事**过了**（从零到跑通），
   但复盘「我自己找出并修掉的 bug」那栏列的 3 条**全部是教练指出的**，
   **ly 自己独立发现的 bug 是 0 条**。
   **这不是写代码能力的问题，是自查能力还没建立**——下周要练的是后者。
   判据：交给教练之前，他自己跑过几条路径、验过哪几个判据。
...
| 2026-08-09 | Week 2 ④「JSON 解析」**不卡门槛**，转 Week 3 Day 1 第一个任务 | 课程欠账不是学习者欠账 —— Week 2 压根没排过 `json.loads`。门槛拦「夹生」，不拦「没教过」 |
| 2026-08-09 | **Week 3 定为双轨 5:2**（Day 1-2 AI 线阶段 2 开头 / Day 3-7 主线阶段一） | 两条轨道都指向 Week 3。AI 线阶段 2 指南给 **4 周**，一周只能做开头（FC 跑通一次，8 大技法不排）；其余给欠得最狠的主线阶段一。ly 拍板 |
| 2026-08-09 | **目录重构推到 Week 3 Day 7**（原定 Day 7 做） | 改路径要连带改 `week1/CLAUDE.md` / 两份 README / 本文件 / `~/.claude` memory 全部引用。开工第一天搬家 = 在最需要专注时制造坏链接 |
| 2026-08-09 | **新增交付格式要求**：交给教练之前先自己跑路径、写下验过哪些判据 | 新瓶颈 4「能写 ≠ 能自查」—— Day 7 盲写脚本过了，但独立发现的 bug 是 **0 条**。判据：Week 3 结束时这栏不能再是 0 |
|  |  |  |
