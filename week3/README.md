...
> **`[x]*` = 教练代填，答案不是 ly 产出的**，验收挪到 Week 4 间隔回顾（沿用 Week 2 ①⑤ 先例）。
> 09-18 教练裁决，逐条依据见 `day7_review.md` 教练批改区。

- [x] 能让模型稳定吐出合法 JSON，并在**解析失败时不崩**
      —— `day1_json.py:72-73`，`response_format={"type":"json_object"}` + `except json.JSONDecodeError`，
      精确到子类没裸 `except`。⚠️ 原答漏了「挡不住哪两种」（形状不对 / 内容瞎编），已在复盘补
- [x] 能说清 Function Calling 四步流程，并答出「**模型不会真的调用工具，执行权在你手里**」
      —— **09-21 ly 拍板放行**。证据 = 本文件 `:47` 排期表那行已写全四步含「**结果塞回**」（commit `e0c7df8`），
      ly 本人指认。⚠️ 留痕：Day 7 原答第 4 步写「模型返回结果」（`day7_review.md:411`），主语给了模型；
      正确表述一直在自己项目的第 47 行，答题时没翻 —— 形状同瓶颈 1
- [x] 能配好 Vite 代理，说清为什么开发要代理、生产不需要
      —— 三句都在点上，「服务端转发」答到。⚠️ 两处留痕：这是 09-18 间隔回顾 R1 **给过答案后的复述**；
      「被 CORS 拦截」已纠正为「被同源策略拦截，CORS 是放宽它的机制」
- [x] 能给 `/summarize` 的 6 字段契约写出 TS `interface`，**不写 `any`**（09-21 ly 拍板放行，原 `[x]*`）
      —— 字段名 6 个全对，**类型全答成了 Python**（`str`/`int`/`float`），而映射表就写在
      `types/api.ts` 的 TODO 注释第 8 行 → **瓶颈 1 第 8 次**。
      `any` 09-18 已清（`unknown` + `axios.isAxiosError<ApiError>` 类型守卫），
      判据：`type-check` 通过 · `eslint src/` **1 error → 0** · `grep ": any" src/` 无命中
- [ ] Vue 版前端端到端跑通（选文件 → 上传 → 出总结 → 四类错误各有对应 UI）
      —— 四条路径的判据与结果已写全（见复盘 ⑤ 的表），但**路径 1/2/4 是教练在服务器上跑的**，
      路径 3 只有口述现象没有判据值。**ly 本人跑满四条这件事未做**，本周硬要求未兑现
- [x] **能说出原生版和 Vue 版的三条本质差别**（← 方案 C 的目的，不是「哪个好看」）（09-21 ly 拍板放行，原 `[x]*`）
      —— 原答三条经核实是**同一条的三个例子**；09-18 ly 说「本质差别我不会」，教练按规则 1 例外代填：
      **谁动手**（命令式/声明式）／**状态住在哪**（DOM 是状态/DOM 是投影）／**元素在不在**（`v-show`/`v-if`）
- [x] **「我自己独立发现的 bug」不是 0** —— **2 条，均在教练指出之前**：
      ① `handleSubmit` 的 `else` 缺 `return`（commit `a8e75a8`，早于 review）
      ② 停后端时 Vite 代理自造 502 → `err.response` 存在 → 走 `case 502` 不走 `else`，
      **推翻了教练写在骨架里的判据**（教练据此实测证实）

> **门槛规则**：全部 ☑ 才解锁 Week 4。**进度慢不是问题，夹生才是问题。**
> （Week 2 的裁决同样适用：门槛拦「夹生」，不拦「没教过」。）
>
> ~~**09-18 裁决：Week 4 暂不解锁。**~~ **09-21 改判：②④⑥ 由 ly 拍板放行**
> （② 依据 `:47` 已有完整四步；④⑥ 原 `[x]*` 一并转正）。
> **仅剩 ⑤ 未兑现** —— 四条路径的数据来源仍是教练在服务器上跑的，
> 「ly 本人跑满四条」这条针对瓶颈 4 的硬要求，改在 Week 4 Day 1 正课里顺带做掉，不再单独卡门槛。
